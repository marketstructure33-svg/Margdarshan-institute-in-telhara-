...
import { Pen, Eraser, Trash2, Mic, MicOff, Maximize, Minimize, Square, Circle, Minus, Undo2, Type, Highlighter, Image as ImageIcon, Download, Grid, AlignJustify } from 'lucide-react';

export type DrawMode = 'pen' | 'eraser' | 'rect' | 'circle' | 'line' | 'text' | 'highlighter' | 'image';
...
export function AdminWhiteboard() {
  ...
  const redraw = () => {
    ...
    const drawAction = (action: DrawAction) => {
      ...
      if (action.type === 'pen' || action.type === 'eraser' || action.type === 'highlighter') {
  ...
  const handleStart = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if (mode === 'image') return; // Handled separately via file upload
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        redraw();
        setMode('pen'); // Switch back to pen
      };
      img.src = dataUrl;
    };
  ...
  const exportBoard = () => {
    ...
    // Create a temporary canvas with white background to export
      ...
      <div ref={containerRef} className={\`flex flex-col gap-4 max-w-6xl mx-auto \${isFullscreen ? 'bg-slate-50 p-6 overflow-y-auto h-screen w-screen max-w-none' : ''}\`}>
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex-wrap gap-4">
          <div className="flex gap-4 items-center flex-wrap">
            ...
            {(mode === 'pen' || mode === 'rect' || mode === 'circle' || mode === 'line' || mode === 'text' || mode === 'highlighter') && (
              <div className="flex gap-2 items-center ml-2 border-l border-slate-200 pl-4">
                {colors.map(c => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
                    className={\`w-6 h-6 rounded-full border-2 transition-all \${color === c ? 'border-slate-400 scale-110 shadow-md' : 'border-transparent'}\`}
